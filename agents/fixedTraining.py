...
class DQNAgent:
    def __init__(self, env, action_size=4, state_size=4, batch_size=1024, gamma=0.70, epsilon=1.0, epsilon_decay=0.99995, epsilon_min=0.01):
        self.env = env
...
def log_decision(log_file, state, valid_actions, chosen_action, reward, winnings, done):
    log_entry = {
        "state": state.tolist() if isinstance(state, torch.Tensor) else state,
        "valid_actions": valid_actions,
